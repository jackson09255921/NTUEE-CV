    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            if len(corners) > 0:
                ...
                dst_corners = np.array(corners[0]).astype(int).reshape(4,2)

                H = solve_homography(ref_corns, dst_corners)

            # TODO: 3.apply backward warp
            # function call to warping()
                xmin, ymin = np.min(dst_corners, axis=0)
                xmax, ymax = np.max(dst_corners, axis=0)
