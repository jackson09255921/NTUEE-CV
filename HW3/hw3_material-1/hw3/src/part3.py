...
if __name__ == '__main__':
    ...
    square_corners = np.array([[0, 0],[w - 1, 0], [w - 1, h - 1], [0, h - 1]])

    dst = np.zeros((h, w, c))
    H = solve_homography(corners1, square_corners)
    output3_1 = warping(secret1, np.zeros((h, w, c), dtype=np.uint8), H, 0 , h, 0, w, direction='b')
    
    dst = np.zeros((h, w, c))
    H = solve_homography(corners2, square_corners)
    output3_2 = warping(secret2, np.zeros((h, w, c), dtype=np.uint8), H, 0 , h, 0, w, direction='b')
